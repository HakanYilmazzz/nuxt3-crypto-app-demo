...
async function fetchData() {
    try {
        const { data } = await useFetch(`https://api.nomics.com/v1/currencies/ticker?key=1da999db0f7e0fa4f70d97f48d6ffe612260cf18&interval=1d,30d&convert=TRY`,{
            method: "GET",
                ...
                <table class="w-3/6 text-center table table-zebra rounded-lg"
                    ...
                    <tbody class="p-2 border-solid border-1  bg-white  text-center ">
                        <tr v-for="(content, index) in contents" class="shadow-lg shadow-cyan-500/50">
                            <td class=" rounded-l-lg"><img :src="content.logo_url" alt="" width="60"
                            ...
                            <td class="rounded-r-lg">{{ parseFloat(content.price).toLocaleString() }} TL</td>
                        </tr>
                    </tbody>
                </table>
            </div>
